...
| SF02 | Aguilar et al. 2008; 101 publications / 102 unique species | source verified; Appendix S1 row materialization pending | materialize and deduplicate Appendix S1 |
| SF03 | Aguilar et al. 2019; 179 plant species | source verified; supporting-information row materialization pending | materialize bibliography and crosswalk G_offspring/F |
...
| SF06 | Aguilar et al. 2024/2025 Supplementary Table S1 | source verified; row materialization pending | materialize publication rows and repeated systems |
| SF07 | Olhnuud et al. 2025 Dryad; 80-study source denominator | source verified; row materialization blocked by Dryad anti-bot download layer in GitHub Actions | revisit only through an official machine-readable route |
...
3. **CF01** — targeted citation expansion has been used for declared G-pair recovery, but the full all-seed backward/forward search has not been completed.

Access blockers:

- **SF01** — public metadata verified but historical KNB/DataONE object bytes are not reproducibly resolvable.
- **SF07** — public Dryad landing metadata verified, but machine downloads return an anti-bot validation page in GitHub Actions.

Still awaiting row materialization:
...
1. materialize SF02, SF03 and SF06 using current public supplements;
2. screen and crosswalk the 38 SF04 publications;
3. restore the missing SF05 outcome-excluded identities where legitimately recoverable;
4. construct the deduplicated cross-frame programme ledger;
