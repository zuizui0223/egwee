# ML009 / PS002 Magnolia stellata recovery result
...
- C = between-population pollen export only if an auditable donor-by-sink population vector could be reconstructed;
...
The supplement does contain a complete donor-by-sink pollen table on the six focal populations, so a population-level between-population pollen-flow vector is structurally reconstructable. That does not rescue ML009 because the common primary exposure gate fails first.
...
Reopen only if the six population-level summed adult-genet basal-area values (or an author-provided equivalent source table) become available. Until then ML009 contributes mechanistic/native-scale evidence, not a fourth admissible EGWEE multilayer cluster.
