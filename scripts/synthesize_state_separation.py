...
def ml020() -> dict:
    erows = rows("evidence/meta_extraction/PS022_aizen_feinsinger_effects_v1.csv")
    crows = rows("evidence/meta_extraction/PS022_aizen_feinsinger_covariance_v1.csv")
    species = ["Atamisquea emarginata", "Cercidium australe", "Prosopis nigra"]
    species_results = []
    for sp in species:
        sr = [r for r in erows if r["species"] == sp]
        assert len(sr) == 2
        effects = {r["endpoint_id"]: float(r["oriented_effect"]) for r in sr}
        assert set(effects) == {"I_pollen_tubes", "F_fruit_set"}
        cov = {
            (r["endpoint_i"], r["endpoint_j"]): float(r["sampling_covariance"])
            for r in crows if r["species"] == sp
        }
        assert len(cov) == 4
        tested = cluster_test(f"ML020:{sp}", effects, cov)
        assert tested["n_pairs"] == 1
        species_results.append(tested)

    programme_p = min(1.0, len(species_results) * min(r["cluster_p_bonferroni"] for r in species_results))
    return {
        "cluster_id": "ML020",
        "n_endpoints": 6,
        "n_species": 3,
        ...
        "cluster_p_bonferroni": programme_p,
        "programme_gate": "bonferroni_across_three_source_explicit_four_site_species",
    }
...
def main() -> None:
    registry_rows = rows("evidence/meta_extraction/multilayer_cluster_registry_v1.csv")
    registry_rows += rows("evidence/meta_extraction/multilayer_cluster_registry_extension_ml020.csv")
    ...
    assert primary_ids == {"ML001", "ML002", "ML003", "ML014", "ML020"}
    assert sum(int(registry[cid]["n_admissible_primary_effects"]) for cid in primary_ids) == 17
    ...
    primary_clusters = [ml001(), ml002(), ml003(), ml014(), ml020()]
    ...
    out = {
        ...
        "claim_ceiling": (
            "primary inference uses five direct Hedges-g programme/study clusters ML001-ML003, ML014 and retrospective ML020; "
            "ML020 counts once although it contains three dependent species subsystems; ML015 is separate Fisher-z gradient generalisation evidence; "
            "no Hedges-g/Fisher-z cross-family Fisher combination is permitted; the full primary rejection remains materially dependent on ML001 Serapias "
            "because the omit-ML001 five-cluster sensitivity does not reject"
        ),
    }
