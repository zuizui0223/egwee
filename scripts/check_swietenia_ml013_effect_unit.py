...
def main() -> None:
    ...
    for token in ("6.1", "8.3", "N_ep = 1.9", "3.6", "single forest population (`S`)", "not converted into primary Hedges-g effects"):
        assert token in text

    admissible = [x for x in registry.values() if x["cluster_status"] == "admissible_multilayer_cluster"]
    assert {x["cluster_id"] for x in admissible} == {"ML001", "ML002", "ML003", "ML014"}
    assert sum(int(x["n_admissible_primary_effects"]) for x in admissible) == 11

    print(
        "Swietenia ML013 effect-unit audit: PASS; strong multilayer programme evidence retained, "
        "single continuous reference prevents ML013 admission; current primary denominator=4/11"
    )
