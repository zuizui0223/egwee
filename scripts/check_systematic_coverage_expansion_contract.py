...
CF01_CANDIDATE_SCREEN_CONTRACT = ROOT / "manuscript/CF01_CANDIDATE_SCREENING_CONTRACT_2026-09-23.md"
...
def main() -> None:
    ...
    cf01_screen_contract = CF01_CANDIDATE_SCREEN_CONTRACT.read_text(encoding="utf-8")
    for token in (
        "frozen before any newly discovered candidate effect magnitude is opened",
        "Ambiguity is not an exclusion criterion",
        "endpoint-to-layer assignment is fixed before calculating the candidate effect",
        "whether admitting the study would move a pair family closer to the five-programme analysis gate",
        "Search completion, not statistical significance",
    ):
        assert token in cf01_screen_contract, token
