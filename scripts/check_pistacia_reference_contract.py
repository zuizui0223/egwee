...
def main() -> None:
    ...
    # The cross-system denominator must remain exactly the already-admitted three clusters.
    admitted = {
        cid
        for cid, row in registry.items()
        if row["cluster_status"] == "admissible_multilayer_cluster"
    }
    assert admitted == {"ML001", "ML002", "ML003"}

    print(
        "ML009 Pistacia reference audit: PASS; 2007 season overlap retained descriptively, "
        "but one fragmented vs one continuous landscape cannot enter the replicated multilayer synthesis"
    )
