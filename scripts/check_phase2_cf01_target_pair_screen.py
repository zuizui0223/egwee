...
def main() -> None:
    ...
    assert cat["quantitative_gate_status"] == "blocked_1997_common_site_identity_and_site_level_F_vector_not_recoverable"
    ...
    assert myrtus["quantitative_gate_status"] == "blocked_population_level_F_or_PL_vector_not_publicly_recoverable"
    ...
    assert spring["quantitative_gate_status"] == "blocked_site_level_Bombus_effort_normalization_not_publicly_recoverable"
    assert int(spring["pair_programme_increment"]) == 0
    assert spring["effect_calculation_opened"] == "no"

    schlotman_stop = SCHLOTMAN_STOP.read_text(encoding="utf-8")
    for token in (
        "21 forest fragments",
        "Table 4",
        ...
        assert has_token(schlotman_stop, token), token
    ...
    catasetum_gate = CATASETUM_GATE.read_text(encoding="utf-8")
    for token in (
        "blocked_1997_common_site_identity_and_site_level_F_vector_not_recoverable",
        "five island sites",
        "five mainland sites",
        "does **not identify which five island sites**",
        "Site (forest type)",
        "Figure 1 labels all 15 study sites",
        "No Hedges-g I or F effect is admitted",
        "effect-unit/recoverability STOP",
    ):
        assert has_token(catasetum_gate, token), token
    ...
    myrtus_stop = MYRTUS_STOP.read_text(encoding="utf-8")
    for token in (
        ...
        assert has_token(myrtus_stop, token), token
